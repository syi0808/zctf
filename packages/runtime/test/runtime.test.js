...
import { BenchReportView } from "../src/bench-report.view.js";
...
test("compiled config is consumed and cached by native code", () => {
  const config = createConfig("medium");
...
test("specialized config compiler covers defaults, known names, ASCII, and UTF-8", () => {
  ...
  assert.ok(
    compileConfig(createConfig("pluginHeavy")).byteLength <
      compileConfig(createConfig("large")).byteLength,
  );
  const defaults = compileConfig(createConfig("defaultHeavy"));
  ...
  const known = compileConfig(createConfig("pluginHeavy"));
